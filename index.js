...
const client = new Client({
  partials: ["CHANNEL"], intents: [Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_MESSAGES, Intents.FLAGS.DIRECT_MESSAGES,
...
async function AddDungeon(charName, realm, region, axios, embedMessage, longName, displayName, dungeonCode, level, top5 = false) {
  try {
    ...
    if (!top5) {
      embedMessage.addField(`${longName}:`, `[${dungeonDetails.Best.Level}](https://www.warcraftlogs.com/reports/${dungeonDetails.Best.Code}#fight=${dungeonDetails.Best.FightID}&type=damage-done) | ${PrintDps(dungeonDetails.Best.amount)} | ${RankScore(dungeonDetails.Best.Rank)} ${dungeonDetails.Best.Rank} %`, true);
    }
    else {
      let DungeonString = "";
      ...
      embedMessage.addField(`${longName}:`, DungeonString, true);
    }
  }
  catch (e) {
...
async function LookupBattlenetCharAvatar(url) {
  ...
  return await axios.get(url).then(res => {
    //console.log(`LookupCharacter statusCode: ${res.status}`);
  ...
  client.on('interactionCreate', async interaction => {
    ...
    try {
      ...
      if (interaction.commandName === 'wcl') {
        ...
        console.log(`Parsed Message understood as: Char: ${charName} on: ${realm} in: ${region} for Level: ${level} Report?: ${reportType}`);
